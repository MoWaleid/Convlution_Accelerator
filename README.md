# Exact CFGLUT5 K16 EdgeFree — 125 MHz

Branch: `feature/k16-cfglut5-edgefree-125mhz`. Based on updated upstream `v1-bringup` commit `fb66066`.
...
This is exact N3/K16/W32 convolution, not Drop-2. All 16 channels run in parallel, engine II=1 when unstalled, pipeline=6 stages. Runtime weights/bias/shift/ReLU remain programmable. [release/hardware.json](release/hardware.json) defines this branch's unique identity. [release/STATUS.md](release/STATUS.md) distinguishes fresh release validation from earlier reference reports.

Canonical RTL is `Convlution_Accelerator.srcs/sources_1/new/`; the complete system BD is `Convlution_Accelerator.srcs/sources_1/bd/accelerator_dma/accelerator_dma.bd`. The fresh build copies all inputs into `work/release_125/project/`. Open that generated `edgefree.xpr`, not an old cached project.
