...
AXI DMA, computes a stride-1, same-mode (zero-padded) 3×3 convolution against 8
programmable channels, and streams signed 16-bit feature maps back to DDR memory.
All arithmetic is bit-exact against an arbitrary-precision Python golden model, and
the complete hardware-software system has been qualified on physical silicon with
188 recorded bit-exact inference frames [S5][S6][S7][S8].
...
(`axi_stream_output_serializer.vhd` [S10]), so the sustained output rate is
**4/K positions per clock**: 0.5 positions/cycle for K=8, 0.25 for K=16.
A 32×32 frame at K=8 requires 2,048 output beats = 2,048 clocks = 20.48 µs of
datapath streaming time (derived). The measured end-to-end wall clock including
DMA and Python-side polling was 0.074–0.121 ms per frame (median 0.076 ms) across
103 consecutive verified frames [S5].
...
**DMA buffer layout** (`software/m4_filebackend.py`, `software/hardware.json` [S9]):
a physically contiguous u-dma-buf allocation (4 MiB at 0x1F100000, sync_mode 1
[see boot record, §13 S14]) is partitioned as TX frame at +0x1000, RX at +0x10000
with 0x8000 capacity, and 64-byte 0xA5-filled guard regions on both sides of RX.
Guards are verified after every transfer to prove the DMA wrote exactly its length.
...
discovery registers, four live transfer counters, a 128-bit BUILD_ID
(`M4N3K8W32-260911` for this release) and the DMA length-width capability [S10:
...
length width **before any parameter write** and rejects mismatches — a rule the
...
**Total: 185 transcript-recorded bit-exact frames across 8 runs, zero mismatches**
(plus the M4 integration bring-up run documented in the project record). Corner-case
...
|---|---|---|---|
| Input image size | 32×32 (single-channel grayscale; 640×480 profile on the roadmap) | px | [S10] |
...
| Latency | 4 cycles compute pipeline; 2,048 output beats ≈ 20.48 µs streaming per frame (derived); 0.076 ms measured end-to-end median | — | [S10], [S5] |
| Throughput | 4/K output positions per cycle sustained @ 100 MHz (0.5 positions/cycle for K=8) | positions/cycle | [S10: serializer], [S5] |
...
| Verification status | bit-exact vs golden model: 185 transcript-recorded board frames across 8 runs, 0 mismatches + full simulation regressions | — | [S4]–[S8] |
...
switching architecture in the project roadmap.
...
FOM = Throughput / (Power × (LUTs + 50·DSPs + 100·BRAMs)), where throughput is the
sustained output rate in positions/cycle. The output serializer packs 4 int16 values
per 64-bit beat, giving 4/K positions per clock (0.5 for K=8). Because this submission
is a **complete system** (accelerator + AXI DMA + interconnects + PS integration +
runtime-reconfiguration software) while some submissions may report a bare core, the
FOM is stated at **two explicitly measured scopes** so that neither extra system cost
nor missing integration can distort the comparison:

| Scope | Throughput | Power | LUTs | DSP | BRAM | FOM |
|---|---|---|---|---|---|---|
| **Accelerator core only** (hierarchical block power + utilization, [S2][S3]) | 0.5 pos/cycle | 0.018 W | 7,967 | 0 | 0 | **0.5 / (0.018 × 7,967) ≈ 3.5×10⁻³** |
| **Full routed system** | 0.5 pos/cycle | 1.765 W | 12,362 | 0 | 3 | **0.5 / (1.765 × 12,662) ≈ 2.2×10⁻⁵** |
...
1. FOM throughput is the sustained output rate in positions per cycle, limited by
   the 64-bit output serializer (4 int16 per beat → 4/K positions per clock).
...
3. Board latency figures include DMA setup and Python-side polling overhead; the
   datapath-only frame time (1,024 cycles ≈ 10.24 µs) is derived, not measured.
...
experiments via the included scripts (`software/m5_qualify.py`, `software/m6_reload.py`)
with `software/hardware.json` as the platform identity.
...
| Board software | `software/conv_lab/`, `software/m4_filebackend.py`, `software/m5_qualify.py`, `software/m6_reload.py`, `software/hardware.json` |
| FPGA reports | timing, power, utilization, DRC, route status (`Convlution_Accelerator.runs/impl_1/*.rpt`, `m4_final_utilization.rpt`) |
| Board-run evidence | `report/evidence/*.txt` (5 transcripts), `debug_captures/` (Sobel renders, ILA captures) |
