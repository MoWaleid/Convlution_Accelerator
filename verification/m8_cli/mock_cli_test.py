...
Covers: same-build run (anchor mode), full-reload run (B32_CFGLUT125), exact-reference
...
# --- stage fixture: clone the synced profile stage so catalog/bundle
# mutations below never touch the persistent mock stage. Firmware resolves
# from the repo: all five canonical .bit.bin images exist locally with
# hashes matching the admitted manifests (S19.7 steps 1-2). ------
import shutil
...
def set_current(profile):
    ...
    catalog = json.loads((MH.M.BASE / "m7_profiles.json").read_text())
    chans, _bundle = MH.M.load_params(
        CLI.release_bundle_dir(profile, catalog),
...
def main():
    ...
    MH.program_profile("A32_CFGLUT125")          # cold-start equivalent: A32 live

    # 1) same-build run, library image, anchor mode, previews on
    set_current("A32_CFGLUT125")
    CLI.main(["run", "--profile", "A32_CFGLUT125", "--frames", "2", "--previews"])
    ...
    set_current("B32_CFGLUT125")
    CLI.main(["run", "--profile", "B32_CFGLUT125", "--frames", "1"])
    rec, _ = newest_record()
    ...
    try:
        CLI.main(["run", "--profile", "A32_CFGLUT125", "--frames", "1", "--image",
                  str(custom)])
        rec, _ = newest_record()
    ...
    # 4) benchmark harness
    set_current("A32_CFGLUT125")
    CLI.main(["benchmark", "--profile", "A32_CFGLUT125", "--frames", "20", "--warmup", "2"])
    rec, _ = newest_record()
    ...
    set_current("A32_CFGLUT125")
    CLI.main(["extremes", "--profile", "A32_CFGLUT125"])
    rec, run_dir = newest_record()
    assert rec["outcome"] == "PASS", rec
    ...
    set_current("A32_CFGLUT125")
    CLI.main(["soak", "--profile", "A32_CFGLUT125", "--frames", "5"])
    rec, _ = newest_record()
    ...
    CLI.load_image_exact = fake_loader
    try:
        CLI.main(["soak", "--profile", "B32_CFGLUT125", "--frames", "2", "--image",
                  str(custom)])
    finally:
        CLI.load_image_exact = real_loader
    rec, _ = newest_record()
    ...
    try:
        try:
            CLI.main(["soak", "--profile", "A32_CFGLUT125", "--frames", "2",
        ...
        try:
            CLI.main(["soak", "--profile", "A32_CFGLUT125", "--frames", "1",
                      "--image", str(big)])
    ...
    try:
        ...
        try:
            with redirect_stdout(out):
                CLI.main(["run", "--profile", "A32_CFGLUT125", "--frames", "1"])
    ...
    real_rel = cat2["releases"]["A32_CFGLUT125"]
    cat2["releases"]["A32_CFGLUT125"] = dict(real_rel, bundle_sha256="0" * 64)
    ...
    cat2["releases"]["A32_CFGLUT125"] = dict(real_rel, shape_id="N3K8W32H33-CVH1")
    cat_p2.write_text(json.dumps(cat2))
    try:
        CLI.main(["run", "--profile", "A32_CFGLUT125", "--frames", "1"])
    ...
    cat2["releases"]["A32_CFGLUT125"] = real_rel
    cat_p2.write_text(json.dumps(cat2))
    CLI.main(["run", "--profile", "A32_CFGLUT125", "--frames", "1"])
    rec, _ = newest_record()
    assert rec["outcome"] == "PASS"
    assert rec["profile"]["release_id"] == "A32_CFGLUT125"
