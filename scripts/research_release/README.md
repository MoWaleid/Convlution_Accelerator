...
- `research_build.tcl` — the Vivado flow: fresh project (`xc7z020clg484-1`),
  explicit source list from the spec, portable ZedBoard board-file discovery,
  isolated profile-clock application, and parameter assertions (effective PS
  clock, DMA config, no System ILA), fail-closed
...
- `research_release.py` — host driver: spec cross-check against
  ...
  `config_pkg.vhd` (six selected-profile constants are each replaced exactly
  once; repo template never mutated), Vivado batch invocation,
  and the IP-08 build-input manifest (`build_manifest.json`: tool version,
  Git commit and tracked-worktree state, every RTL source, release driver,
  build Tcl, spec, catalog, BD, XDC and artifact SHA-256, results, and
  BOARD_VALIDATION).
...
parameter assertions. The build directory must be absent or contain exactly
the rendered `src/config_pkg.vhd` created by `prepare`; stale projects and
...
- `create_accelerator_dma_bd.tcl` is retired as non-authoritative for
  ...
  its structural parameters, then applies the checked profile clock only to
  the isolated imported copy.
