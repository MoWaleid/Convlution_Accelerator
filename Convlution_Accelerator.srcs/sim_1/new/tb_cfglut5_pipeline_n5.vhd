-- Seven-stage N=5 regression: window distribution, two generated bitheap
-- boundaries, and four arithmetic/postprocessing registers.
library ieee;
...
entity tb_cfglut5_pipeline_n5 is end;
architecture sim of tb_cfglut5_pipeline_n5 is
    ...
    signal pixels : pixel_array_t(0 to C_N * C_N - 1) :=
        (others => (others => '0'));
...
begin
    ...
    monitor : process
        variable valids : std_logic_vector(6 downto 0) := (others => '0');
    ...
    begin
        ...
        elsif enabled then valids := valids(5 downto 0) & vin;
        end if;
        wait for 1 ns;
        assert vout = valids(6)
            report "Seven-stage N=5 CE-relative valid latency mismatch"
            severity failure;
        if resetn = '1' and not enabled then
            assert vout = old_valid and result(0) = last_result
                report "N=5 pipeline changed while CE was low"
                severity failure;
        end if;
        if vout = '1' then
            assert to_integer(signed(result(0))) = expected
                report "N=5 postprocessing mismatch expected="
                    & integer'image(expected) & " actual="
                    & integer'image(to_integer(signed(result(0))))
                severity failure;
    ...
    begin
        ...
        resetn <= '1';
        -- Seven packed words cover all 25 zero coefficients, including the
        -- one-byte tail word used by the N=5 register map.
        for word in 0 to 6 loop
            wait until falling_edge(clk);
            ...
            for r in 0 to 1 loop
                for b in BIASES_TO_TEST'range loop
                    ...
                    biases(0) <= std_logic_vector(
                        to_signed(BIASES_TO_TEST(b), CFG_BIAS_WIDTH));
                    shifts(0) <= std_logic_vector(
                        to_unsigned(sh, CFG_SHIFT_WIDTH));
                    if r = 1 then relus(0) <= '1'; else relus(0) <= '0'; end if;
                    expected <= reference(BIASES_TO_TEST(b), sh, r);
                    for tick in 0 to 19 loop
                        wait until falling_edge(clk);
                        if tick = 2 or tick = 3 or tick = 9 then ce <= '0';
                        else ce <= '1'; end if;
                        if tick < 8 then
                            vin <= '1';
                    ...
                    wait until falling_edge(clk);
                    assert vout = '0' report "N=5 pipeline did not drain"
                        severity failure;
                    assert received = sent
                        report "N=5 pipeline dropped/duplicated output"
                        severity failure;
                end loop;
            end loop;
        end loop;
        ...
        wait until falling_edge(clk);
        resetn <= '1';
        for tick in 0 to 8 loop wait until falling_edge(clk); end loop;
        assert vout = '0' report "N=5 reset left a stale result"
            severity failure;
        report "PIPELINE_N5_125_PASS shifts=32 bias_cases=16 relu_modes=2 outputs="
    ...
    begin
        wait for 350 us;
        assert false report "N=5 pipeline regression timeout" severity failure;
    end process;
end;
