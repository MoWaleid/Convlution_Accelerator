...
architecture rtl of conv_engine is
    ...
    signal cfg_pending : std_logic := '0';
    constant C_CHANNELS_PER_BANK : positive := 4;
    constant C_WINDOW_BANKS : positive := (C_K + C_CHANNELS_PER_BANK - 1) / C_CHANNELS_PER_BANK;
    type window_bank_array_t is array (0 to C_WINDOW_BANKS-1) of pixel_array_t(0 to C_TAPS-1);
    signal window_banks : window_bank_array_t := (others => (others => (others => '0')));
    signal window_valid_q : std_logic := '0';
    -- Four local distribution copies, not replicated line-history storage.
    -- Preserve the physical banks against equivalent-register merging.
...
begin
    ...
    -- falls. Do not admit a new write/START until the CFGLUT has consumed it.
    cfg_ready <= not (cfg_active or cfg_pending);

    distribution_process : process(clk)
    begin
        if rising_edge(clk) then
            if cfg_resetn = '0' then cfg_pending <= '0';
            else cfg_pending <= cfg_active; end if;
            if resetn = '0' then
                window_banks <= (others => (others => (others => '0')));
                window_valid_q <= '0';
            elsif ce = '1' then
                for bank in 0 to C_WINDOW_BANKS-1 loop
                    window_banks(bank) <= window_in;
                end loop;
                window_valid_q <= valid_in;
            end if;
        end if;
    end process;
    ...
    begin
        if rising_edge(clk) then
            if cfg_resetn = '0' then
                ...
                assert cfg_active = '0' and cfg_pending = '0'
    ...
    gen_channels : for channel in 0 to C_K - 1 generate
        signal cfg_ce_taps : std_logic_vector(0 to C_TAPS - 1) := (others => '0');
        signal cfg_data_local : std_logic_vector(5 downto 0) := (others => '0');
    begin
        -- Register serial data and the selected bank enable together. The
        -- per-channel load enable prevents merging all data registers into a
        -- single high-fanout driver. No convolution-output cycle is added here.
        config_distribution : process(clk)
        begin
            if rising_edge(clk) then
                if cfg_resetn = '0' then
                    cfg_ce_taps <= (others => '0');
                    cfg_data_local <= (others => '0');
                else
                    cfg_ce_taps <= (others => '0');
                    if cfg_active = '1' and cfg_channel_index = channel then
                        cfg_data_local <= cfg_data;
                        cfg_ce_taps(cfg_tap_index) <= '1';
                    end if;
                end if;
            end if;
        end process;
        ...
        channel_inst : entity work.conv_channel
            ...
            port map (
                ...
                resetn     => resetn,
                ce         => ce,
                window_in  => window_banks(channel / C_CHANNELS_PER_BANK),
                cfg_ce     => cfg_ce_taps,
                cfg_data   => cfg_data_local,
                bias_in    => bias_all(channel),
                shift_in   => shift_all(channel),
                relu_en_in => relu_en_all(channel),
                valid_in   => window_valid_q,
