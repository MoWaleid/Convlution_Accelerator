...
--   S4  round-half-up, saturation and optional ReLU
-- ============================================================================
...
architecture rtl of conv_channel is
    ...
    signal result_s4 : std_logic_vector(C_OUT_W - 1 downto 0) :=
        (others => '0');
    ...
    begin
        if rising_edge(clk) then
            if resetn = '0' then
                ...
                -- S4: established exact post-processing contract.
                ...
                shifted := shift_right(acc_s3, shift_val);
                half_bit := '0';
                if shift_val > C_FULL_W then
                    half_bit := acc_s3(C_FULL_W - 1);
                elsif shift_val /= 0 then
                    half_bit := acc_s3(shift_val - 1);
                end if;
                if half_bit = '1' then
                    shifted := shifted + 1;
                end if;
